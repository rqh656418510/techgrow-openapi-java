...
@Transactional
@SpringBootTest
public class WechatAccessTokenServiceTest {
	...
	@Rollback(value = false)
	public void cache() {
		WechatAccessToken accessToken1 = accessTokenService.get();
		log.info("{}", JSON.toJSONString(accessToken1));
		WechatAccessToken accessToken2 = accessTokenService.get();
		log.info("{}", JSON.toJSONString(accessToken2));
	}
